        ...
        if request.POST['password1'] == request.POST['password2']:
            try:
                ...
                return render(request, 'todoapp/signupuser.html',
                              {'form': UserCreationForm(), 'error': 'This name is already exist, choose another one.'})
        else:
...
def loginuser(request):
    if request.method == 'GET':
        ...
        if user is None:
            return render(request, 'todoapp/loginuser.html', {'form': AuthenticationForm(), 'error':'User and password did not match.'})
        else:
            login(request, user)
            return redirect('currenttodos')
...
def createtodo(request):
    if request.method == 'GET':
        ...
        try:
            ...
            return render(request, 'todoapp/createtodo.html', {'form': TodoForm(), 'error': 'Bad data has been given.'})
